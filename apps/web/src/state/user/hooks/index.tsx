...
export function useFeeDataWithGasPrice(chainIdOverride?: number): {
  ...
  const { data } = useFeeData({
    chainId,
    enabled: chainId !== ChainId.BYTE_TESTNET && chainId !== ChainId.BSC_TESTNET,
    watch: true,
  })
...
export function toV2LiquidityToken([tokenA, tokenB]: [ERC20Token, ERC20Token]): ERC20Token {
  return new ERC20Token(tokenA.chainId, Pair.getAddress(tokenA, tokenB), 18, 'JIM-LP', 'ByteSwap LPs')
}
