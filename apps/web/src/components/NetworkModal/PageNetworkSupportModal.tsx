...
export function PageNetworkSupportModal() {
  ...
  return (
    <Modal title={title || t('Check your network')} hideCloseButton headerBackground="input">
      <Grid style={{ gap: '16px' }} maxWidth="360px">
        ...
        {canSwitch ? (
          <Button
            ...
            isLoading={isLoading}
            onClick={() => (isWrongNetwork ? switchNetworkLocal(ChainId.BYTE_TESTNET) : switchNetworkAsync(ChainId.BYTE_TESTNET))}
          >
            {t('Switch to %chain%', { chain: 'XChain' })}
          </Button>
        ) : (
