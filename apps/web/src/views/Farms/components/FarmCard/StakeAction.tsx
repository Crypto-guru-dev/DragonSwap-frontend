...
const StakeAction: React.FC<React.PropsWithChildren<FarmCardActionsProps>> = ({
  ...
  const [onPresentDeposit] = useModal(
    <FarmWidget.DepositModal
      ...
      showCrossChainFarmWarning={chainId !== ChainId.BYTE_TESTNET && chainId !== ChainId.BSC_TESTNET}
  ...
  const renderStakingButtons = () => {
    return stakedBalance.eq(0) ? (
      <Button variant='primary' onClick={onPresentDeposit} disabled={isStakeReady}>
        {t('Stake LP!!')}
      </Button>
    ) : (
      <IconButtonWrapper>
        <IconButton mr="6px" variant="primary" disabled={pendingFarm.length > 0} onClick={onPresentWithdraw}>
          <MinusIcon color="white" width="14px" />
        </IconButton>
        <IconButton variant="primary" onClick={onPresentDeposit} disabled={isStakeReady || isBloctoETH}>
