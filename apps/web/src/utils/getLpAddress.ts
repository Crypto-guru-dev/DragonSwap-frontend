...
const getLpAddress = memoize(
  (token1: string | ERC20Token | undefined, token2: string | ERC20Token | undefined, chainId: number = ChainId.SEPOLIA) => {
    ...
    if (typeof token1 === 'string' || token1 instanceof String) {
      ...
      token1AsTokenInstance = new ERC20Token(chainId, checksummedToken1Address, 18, 'DRX-LP')
    }
    if (typeof token2 === 'string' || token2 instanceof String) {
      ...
      token2AsTokenInstance = new ERC20Token(chainId, checksummedToken2Address, 18, 'DRX-LP')
    }
