...
const getLpAddress = memoize(
  (token1: string | ERC20Token | undefined, token2: string | ERC20Token | undefined, chainId: number = ChainId.BYTE_TESTNET) => {
    ...
    if (typeof token1 === 'string' || token1 instanceof String) {
      ...
      token1AsTokenInstance = new ERC20Token(chainId, checksummedToken1Address, 18, 'JIM-LP')
    }
    if (typeof token2 === 'string' || token2 instanceof String) {
      ...
      token2AsTokenInstance = new ERC20Token(chainId, checksummedToken2Address, 18, 'JIM-LP')
    }
